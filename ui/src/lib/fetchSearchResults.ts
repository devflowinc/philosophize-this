  ...
  async (
    ...
    try {
      ...
      if (episodeRangeMin !== undefined || episodeRangeMax !== undefined) {
        ...
        if (episodeRangeMin !== undefined)
          episodeFilter.range.gte = episodeRangeMin;
        if (episodeRangeMax !== undefined && episodeRangeMax !== 0)
          episodeFilter.range.lte = episodeRangeMax;
      ...
      const episodes = (data.score_chunks || []).map(
        (scoreChunk: any): Episode => ({
          ...
          title:
            scoreChunk?.metadata?.[0]?.metadata?.episode_title ||
            "Untitled Episode",
          date: scoreChunk?.metadata?.[0]?.time_stamp
            ? new Date(scoreChunk.metadata[0].time_stamp).toLocaleDateString(
                "en-US",
                {
                  month: "long",
                  day: "numeric",
            ...
            : "Unknown Date",
          chunk:
            scoreChunk?.metadata?.[0]?.chunk_html || "No content available",
          episodeLink: scoreChunk?.metadata?.[0]?.link || "",
          transcriptLink:
            scoreChunk?.metadata?.[0]?.metadata?.transcript_link || "",
          episodeNumber:
            scoreChunk?.metadata?.[0]?.metadata?.episode_number || "",
          paragraphNumber:
            scoreChunk?.metadata?.[0]?.metadata?.paragraph_number || "",
          paragraphCountGroup:
